...
function loadComponent(componentPath: string, routeName: string): () => Promise<any> {
  ...
  const fullPathWithIndex = `/src/views${componentPath}/index.vue`
 ...
 * 处理普通路由
 */
function handleNormalRoute(converted: ConvertedRoute, component: any, routeName: string): void {
  if (component) {
    // 如果component是函数，直接使用
