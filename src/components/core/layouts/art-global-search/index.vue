...
<script lang="ts" setup>
  ...
  // 用户 store 可能没有内置 searchHistory，这里做兼容兜底
  const historyResult = shallowRef<AppRouteRecord[]>([])
  const userRefs: any = storeToRefs(userStore) as any
  if (userRefs.searchHistory) {
    historyResult.value = userRefs.searchHistory.value || []
  }
  ...
  // 历史记录管理
  const updateHistory = () => {
    // 如果 store 暴露了 setSearchHistory 则回写；否则仅保存在本组件内存
    const anyStore: any = userStore as any
    if (typeof anyStore.setSearchHistory === 'function') {
      anyStore.setSearchHistory(historyResult.value)
    }
  }
