      ...
      <el-container class="page-body">
        ...
        <el-main class="main-col">
          ...
          <div v-else class="empty-state">
            <el-empty description="请从左侧选择一条会议记录">
    ...
    >
      ...
      <el-form :model="metaForm" label-width="90px" class="meta-form">
        <el-form-item label="标题" required>
          <el-input v-model="metaForm.title" placeholder="请输入会议标题" size="large" />
        </el-form-item>
        ...
        <el-form-item label="分类">
          <el-select v-model="metaForm.category" placeholder="选择分类" size="large">
            ...
            <el-option label="对外" value="对外" />
            <el-option label="对内" value="对内" />
          </el-select>
        </el-form-item>
        ...
        <div v-if="currentArticle" class="article-meta-section">
          <el-card shadow="never" class="meta-card">
            ...
            <div class="meta-content">
              <!-- 文章分类 -->
              <div v-if="currentArticle.category" class="meta-item">
                ...
                <div class="meta-value">
                  <el-tag
                    size="small"
                    :type="getCategoryTagType(currentArticle.category)"
                    effect="plain"
                    class="meta-tag"
                  >
                    {{ currentArticle.category }}
      ...
      <div class="batch-manage-container">
        ...
        >
          ...
          >
            ...
            <el-option label="周会记录" value="周会记录" />
            <el-option label="项目讨论" value="项目讨论" />
...
<script setup lang="ts">
  ...
  const loadArticles = async () => {
    try {
      loading.value = true
      const response = await articlesApi.list({ page: 1, page_size: 200, type: 'meeting' })
      articles.value = response.items || []
      buildTree()
    } catch (error) {
      console.error('加载会议记录列表失败:', error)
      ElMessage.error('加载会议记录列表失败')
    } finally {
      loading.value = false
    }
  }
  ...
  const copyArticleLink = async () => {
    ...
    const articleUrl = `${baseUrl}/login#/articles/meeting?articleId=${rightClickedArticle.value.id}`
  ...
  const saveMeta = async () => {
    ...
    try {
      ...
      console.error('更新会议记录信息失败:', error)
      ElMessage.error('更新会议记录信息失败')
    } finally {
      saving.value = false
    }
  }
  ...
  const startEdit = async () => {
    ...
    try {
      ...
      console.log('🔒 [会议记录] 文章已锁定:', currentArticle.value.id)
  ...
  const cancelEdit = async () => {
    ...
    try {
      ...
      console.log('🔓 [会议记录] 文章已解锁:', currentArticle.value.id)
    } catch (error) {
  ...
  const saveEdit = async () => {
    ...
    try {
      ...
      try {
        await articlesApi.unlock(currentArticle.value.id)
        console.log('🔓 [会议记录] 文章已解锁（保存后）:', currentArticle.value.id)
      } catch (error) {
        console.error('解锁文章失败:', error)
      }

      ElMessage.success('会议记录内容更新成功')
      isEditing.value = false
  ...
  const deleteArticle = async () => {
    ...
    try {
      await ElMessageBox.confirm('确定要删除这条会议记录吗？', '确认删除', {
        type: 'warning',
...
<body>
  ...
  const loadUsersAndDepts = async () => {
    try {
      ...
      console.log('[会议记录] 用户选项数量:', userOptions.value.length)
      console.log('[会议记录] 部门选项:', deptOptions.value)
      ...
      try {
        ...
        console.log('[会议记录] 角色选项数量:', roleOptions.value.length)
      } catch (roleError) {
        console.error('[会议记录] 加载角色列表失败:', roleError)
        roleOptions.value = []
      }
    } catch (e) {
      console.error('[会议记录] 加载失败:', e)
      userOptions.value = []
      deptOptions.value = []
      roleOptions.value = []
    }
  ...
  watch(
    ...
    (newRoles) => {
      ...
      console.log('✅ [MeetingNotes] 已自动选择', selectedUserIds.length, '个成员')
    },
    { deep: true }
  )
  ...
  const handleBeforeUnload = (event: BeforeUnloadEvent) => {
    if (isEditing.value && currentArticle.value) {
      ...
      if (navigator.sendBeacon) {
        ...
        console.log('🔓 [会议记录] 使用 sendBeacon 解锁文章')
      } else {
        ...
        console.log('🔓 [会议记录] 使用同步 XHR 解锁文章')
      }
    }
  }
