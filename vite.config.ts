...
export default ({ mode }: { mode: string }) => {
  ...
  // 如果没有设置API代理URL，默认使用本地后端
  const API_PROXY_URL = VITE_API_PROXY_URL || 'http://localhost:8000'
  ...
  return defineConfig({
    ...
    server: {
      ...
      proxy: {
        // Yjs WebSocket 协作服务器代理（必须在 /api 之前）
        '/api/collaboration/yjs': {
          target: 'http://localhost:1234', // 注意：这里用 http，不是 ws
          ws: true, // 启用 WebSocket 代理
          changeOrigin: true,
          // 保持完整路径，Yjs 服务器监听 /api/collaboration/yjs
          configure: (proxy) => {
            proxy.on('error', (err, req, res) => {
              console.log('❌ [Yjs WS Proxy Error]:', err.message)
            })
            proxy.on('proxyReqWs', (proxyReq, req, socket) => {
              console.log('🔌 [Yjs WS] 代理 WebSocket 连接:', req.url)
            })
            proxy.on('open', (proxySocket) => {
              console.log('✅ [Yjs WS] WebSocket 连接已建立')
            })
            proxy.on('close', (res, socket, head) => {
              console.log('🔌 [Yjs WS] WebSocket 连接已关闭')
            })
          }
        },
        // 通用 API 代理
        '/api': {
          ...
          rewrite: (path) => {
            ...
            console.log(`🔄 [Proxy] ${path} -> ${API_PROXY_URL}${newPath}`)
            return newPath
          },
    ...
    css: {
      preprocessorOptions: {
        ...
        scss: {
          api: 'modern-compiler',
          // 静默依赖中的Sass弃用警告（如 @import）
          quietDeps: true as any,
